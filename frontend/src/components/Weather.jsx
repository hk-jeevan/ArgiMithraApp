...
export default function Weather() {
  ...
  // ✅ FIXED CURRENT LOCATION FUNCTION
  const getCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setError('Please enable GPS and allow location access!')
        setLoading(false)
      },
      {
              ...
              {airQuality?.list?.[0] && (
                <div className="highlight-card air-quality">
                  ...
                  <div className="aqi-details">
                    <div className="aqi-item"><span>PM2.5</span><span>{Math.round(airQuality?.list?.[0]?.components?.pm2_5 || 0)}</span></div>
                    <div className="aqi-item"><span>PM10</span><span>{Math.round(airQuality?.list?.[0]?.components?.pm10 || 0)}</span></div>
                    <div className="aqi-item"><span>NO2</span><span>{Math.round(airQuality?.list?.[0]?.components?.no2 || 0)}</span></div>
                    <div className="aqi-item"><span>O3</span><span>{Math.round(airQuality?.list?.[0]?.components?.o3 || 0)}</span></div>
                  </div>
                </div>
              )}
